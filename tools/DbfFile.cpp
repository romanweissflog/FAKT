...
void DbfFile_c::DumpAll(const char *szDestFileName)
{
    ...
    for (size_t i = 0; i < vecRecords.size(); ++i)
    {
      ...
      if (vecBuffer[0] == vecBuffer[1] || vecBuffer[0] == '0' || vecBuffer[1] == ' ')
      {
        entry.push_back(std::string(" "));
      }
      else
      {
        entry.push_back(std::string(&vecBuffer[1], lastLetter));
      }
