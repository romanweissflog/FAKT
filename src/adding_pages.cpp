...
InvoicePage::InvoicePage(Settings *settings, QSqlQuery &query, QWidget *parent)
  : QDialog(parent)
...
void InvoicePage::keyPressEvent(QKeyEvent *ev)
{
  if (ev->key() == Qt::Key_Enter || ev->key() == Qt::Key_Return)
    return;
  QDialog::keyPressEvent(ev);
...
void InvoicePage::CopyData(QString txt)
{
  if (m_ui->copyBox->currentIndex() == 0)
  {
    return;
